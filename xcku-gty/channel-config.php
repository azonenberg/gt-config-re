...
$lines = file("$basepath/synth/gtwizard_ultrascale_0_gtye4_channel_wrapper.v");
...
foreach($lines as $line)
{
	...
	{
		case 0:
			if(strpos($line, 'channel #('))
				$state = 1;
			break;

		case 1:
			if(strpos($line, 'channel_inst'))
				$state = 2;
			else
				DoParameter($line);
			break;
...
function DoParameter($line)
{
	...
	$fields = explode('(', $line);
	$pname = trim($fields[0]);
	if($pname == '.NUM_CHANNELS')
		return;
	$pname = str_replace('.GTYE4_CHANNEL_', '', $pname);
	$pval = trim($fields[1]);
	$pval = explode(')', $pval)[0];
